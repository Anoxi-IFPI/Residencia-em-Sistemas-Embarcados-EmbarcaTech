...
void vLeituraBotaoA(void *pvParameters) {
    ...
    for (;;) {
        ...
        xQueueSend(xFila_BotaoA, &estado_botao, 0);
...
void vProcessaBotaoA(void *pvParameters) {
    ...
    for (;;) {
        if (xQueueReceive(xFila_BotaoA, &estado_recebido, portMAX_DELAY) == pdTRUE) {
            if (estado_recebido) {
...
int main() {
    ...
    xFila_BotaoA = xQueueCreate(10, sizeof(bool));
